...
shared (init_msg) persistent actor class NftCanister() : async (ICRC7.Service.Service) = this {
  ...
  public shared (msg) func mint(to : ICRC7.Account) : async [ICRC7.SetNFTResult] {
    let setNftRequest : ICRC7.SetNFTItemRequest = {
      ...
      metadata = #Map([
        ("tokenUri", #Text(DefaultConfig.tokenURI)),
        ("name", #Text("Himalayan Masterpiece #" # Nat.toText(nextTokenId))),
        ("description", #Text("An authentic artwork from the Himalayan Masters Art collection, showcasing traditional and contemporary artistic excellence.")),
        ("artist", #Text("Himalayan Master Artist")),
        ("medium", #Text("Digital Art")),
        ("year", #Text("2025")),
        ("collection", #Text("Himalayan Masters Art")),
        ("attributes", #Map([
          ("style", #Text("Traditional Himalayan")),
          ("region", #Text("Himalayas")),
          ("rarity", #Text("Unique")),
          ("authenticity", #Text("Verified"))
        ]))
      ]);
      owner = ?to;
      override = false;
